...
def read_dat(filename):
    ...
    try:
        params = {"visc_rat": visc_rat, "vol_rat": vol_rat, "alpha": alpha, "shear": shear, "time": time, "nvert": nvert, "nface": nface, "header_block": header_block}
...
def calc_moment_inertia_tensor(x):
    ...
    output:
        touple of (moment of inertia tensor, the eigenvalues, and eigenvectors)
    """
    (nvert, __) = x.shape
...
def add_perturbation(x, eps):
    ...
    input:
        x : 2D list of vertex coordinate data, already rotated by eigenvectors of moment of inertia tensor
        eps : the perturbation intensity
    returns:
        numpy array of the the perturbed vertex data
    """
    span = np.amax(x[:, 0]) - np.amin(x[:, 0])
    r = np.sqrt(np.square(x[:, 1]) + np.square(x[:, 2]))
    # add perturbation
    x[:, 1] += eps * np.sin(2*np.pi*x[:, 0]/span) * x[:, 1]/r
    x[:, 2] += eps * np.sin(2*np.pi*x[:, 0]/span) * x[:, 2]/r
    return x


def rotate_mesh(x):
    """
    rotates the mesh to be aligned with the x-axis
    """
