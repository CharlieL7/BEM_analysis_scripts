...
def read_dat(filename):
    ...
    try:
        params = {"visc_rat": visc_rat, "vol_rat": vol_rat, "alpha": alpha, "shear": shear,
...
def calc_moment_inertia_tensor(x):
    ...
    # rotating on x-axis by -pi/2 so x-y plane is still planar flow
    rot_mat = np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]])
    eigvecs = np.matmul(eigvecs, rot_mat)
