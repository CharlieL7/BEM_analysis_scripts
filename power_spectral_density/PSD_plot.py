...
class PSD:
    ...
    def plot_corr_PSD(self, in_dir, out_filename):
        ...
        ax_corr.set_xlabel("lag (ts)")
        ax_corr.set_ylabel("ACF")
        ax_corr.grid(True)
        fig_corr.savefig("{}_corr.pdf".format(out_filename), format="pdf")
        plt.close(fig_corr)

        fig_PSD_ln = plt.figure()
        ax_PSD_ln = fig_PSD_ln.add_subplot(111)
        ax_PSD_ln.plot(freq / self.De, np.real(fourier), ".")
        ax_PSD_ln.set_yscale("log")
        ax_PSD_ln.set_xscale("log")
        ax_PSD_ln.set_title("PSD [De={0}, Ca={1}]".format(self.De, self.Ca))
        ax_PSD_ln.set_xlabel(r"Frequency $\times$ T ($\frac{f}{De}$)")
        ax_PSD_ln.set_ylabel("Amplitude")
        fig_PSD_ln.savefig("{}_PSD_ln.pdf".format(out_filename), format="pdf")
        plt.close(fig_PSD_ln)
